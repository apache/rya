...
public class QueryResultUpdater {
     ...
     */
    public void updateQueryResults(final TransactionBase tx, final VisibilityBindingSet childBindingSet, final QueryMetadata queryMetadata)
            throws Exception {
        checkNotNull(tx);
        checkNotNull(childBindingSet);
        checkNotNull(queryMetadata);

        if (AggregationStateUtil.checkAggregationState(tx, childBindingSet, queryMetadata)) {

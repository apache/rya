...
/**
 * This class provides users with the ability to issue reified queries to Rya.  As opposed to a single triple
 * representing a statement, a reified query consists of a number of triples that all describe the same
 * statement.  For example, instead of having the single statement (http://Bob, http://worksAt,http://CoffeeShop),
 * the reified statement representing this triple would be the collection of triples: {(_blankNode, RDF.TYPE, RDF.STATEMENT),
 * (_blankNode, RDF.SUBJECT, http://Bob), (_blankNode, RDF.PREDICATE, http://worksAt), (_blankNode, RDF.OBJECT, http://CoffeeShop)}.
 * The advantage of expanding the statement into a collection of triples in this way is that additional assertions can be made about the
 * statement.  For example, we could use the triple (_blankNode, <http://createdOn>, <http://date#1/2/17>) to indicate that the statement
 * was created on 1/2/17.  The drawback of reification is that is it inefficient.  It takes four triples to specify what was originally
 * specified with one triple.  So reificiation is expensive from a storage perspective.  It is also expensive from a query perspective in
 * that three joins are required to evaluate a query that is reduced to a single scan in non-reified form.
 * 
 * This class provides Rya with the ability to issue reified queries even though statements are not reified.  Each {@link RyaStatement}
 * contains a {@link StatementMetadata} field that allows users to store additional metadata about a given statement.  When a user
 * issues a reified query (possibly containing StatementPatterns about metadata for that statement), the {@link StatementPattern}s for
 * that parsed query are used to create this class.  Upon construction, the StatementMetadataNode validates that the
 * collection of StatementPatterns represents a reified query, and then evaluates the reified query using a single scan over a range
 * determined by the subject, predicate, and object portions of the reified query.  If additional metadata properties are specified in
 * the reified query, the results of the initial scan are filtered client side by comparing the user specified properties with the
 * StatementMetadata extracted from each of the results.  This class allows users to issue queries about RyaStatements and any contextual
...
public class StatementMetadataNode<C extends RdfCloudTripleStoreConfiguration> extends ExternalSet implements ExternalBatchingIterator {
    ...
    private static final RyaURI SUBJ_ID_URI = new RyaURI(RDF.SUBJECT.toString());
    private static final RyaURI PRED_ID_URI = new RyaURI(RDF.PREDICATE.toString());
    private static final RyaURI OBJ_ID_URI = new RyaURI(RDF.OBJECT.toString());
    private static final RyaURI STATEMENT_ID_URI = new RyaURI(RDF.STATEMENT.toString());
        ...
        for (final StatementPattern pattern : patterns) {
            ...
            if (predicate.equals(TYPE_ID_URI)) {
                ...
                if (statementID.equals(STATEMENT_ID_URI)) {
                   ...
                   //contains more than one Statement containing TYPE_ID_URI as Predicate
    ...
    /**
     * Constructs a {@link StatementPattern} from the StatementPatterns representing a reified query.
     * This StatementPattern has as a subject, the object of the StatementPattern containing the predicate
     * {@link RDF#SUBJECT}. This StatementPattern has as predicate, the object of the StatementPattern containing the predicate
     * {@link RDF#PREDICATE}. This StatementPattern has as an object, the object of the StatementPattern containing the predicate
     * {@link RDF#OBJECT}.  This method also builds a map between all predicates that are not of the above type and the object {@link Var}s
     * they are associated with.  This map contains the user specified metadata properties and is used for comparison with the
     * metadata properties extracted from RyaStatements passed back by the {@link RyaQueryEngine}.
     * 
    ...
    /**
     * This method pairs each {@link BindingSet} in the specified collection with the StatementPattern constraints
    ...
    /**
     * Uses StatementPattern constraints to form a RyaStatement, and fills in any null values with
     * {@link BindingSet} values corresponding to the variable for that position.  
     * 
     * @param bs
     * @return RyaStatement whose values are determined by StatementPattern and BindingSet constraints
     */
    ...
    /**
     * Assigns BindingSet values for any {@link Var} whose {@link Value} is null.
     * Returns the {@link Value} associated with Var (if it has one), otherwise returns the BindingSet Value
     * corresponding to {@link Var#getName()}.  If no such Binding exits, this method returns null.
     * 
     * @param var
     * @param bindings
     * @return Value 
    ...
    /**
     * This is an {@link CloseableIteration} class that serves a number of purposes.  It's primary purpose is to
     * filter a CloseableIteration over {@link Map.Entry<RyaStatement,BindingSet>} using a specified property Map
     * from {@link RyaURI} to {@link org.openrdf.query.algebra.Var}.  This Iteration iterates over the Entries in the user specified Iteration,
     * comparing properties in the {@link StatementMetadata} Map contained in the RyaStatements with the property Map for this class.
     * If the properties match, a {@BindingSet} is formed from the RyaStatement/Properties and joined (if possible) with the
     * BindingSet taken from the Map.Entry.  If the RyaStatement/Property BindingSet cannot be formed or joined the the Entry BindingSet,
     * the Entry in the user specified Iteration is filtered out.  So this class converts Iterations, filters according to the
     * specified property Map, and joins the BindingSet formed from the RyaStatements/Properties with the Entry BindingSet. 
}.   
     *
     */
    class PropertyFilterAndBindingSetJoinIteration implements CloseableIteration<BindingSet, QueryEvaluationException> {
        ...
        public PropertyFilterAndBindingSetJoinIteration(CloseableIteration<? extends Entry<RyaStatement, BindingSet>, 
                RyaDAOException> statements, Map<RyaURI, Var> properties, StatementPattern sp) {
        ...
        /**
         * Fast-forwards Iteration to next valid Entry and builds the BindingSet.
         * 
         * @return BindingSet 
        ...
        /**
         * Builds BindingSet from Entry if possible. Otherwise returns an empty Optional if no
         * valid BindingSet can be built.  Valid BindingSet can be built if this class's
         * property Map is consistent with {@link StatementMetadata} properties for the specified
         * RyaStatement and if the BindingSet built form the StatementMetadata properties can be
         * joined with specified BindingSet. 
         * 
         * @param statement - RyaStatement
         * @param bindingSet - BindingSet
         * @return - Optional containing BindingSet is a valid BindingSet could be built
         */
        ...
        /**
         * Verifies whether this class's property Map is consistent with StatementMetadata properties
         * for specified RyaStatement.  If consistent, this method builds the associated BindingSet otherwise
        ...
        /**
         * Builds the BindingSet from the specified RyaStatement by using the StatementPattern
         * for this class.  This method checks whether StatementPattern has a {@link Value} for each
         * position {@link org.openrdf.query.algebra.Var} (Subject, Predicate, Object).  If it doesn't have a Value, a Binding is created
         * from the RyaStatement using the {@link RyaType} for the corresponding position (Subject, Predicate, Object).
         * 
         * @param statement
         * @return BindingSet
         */

...
public class MongoPrecomputedJoinIndexer extends BasePcjIndexer {
    ...
    public MongoPcjDocuments getPcjStorage(final Configuration conf) {
        checkArgument(conf instanceof StatefulMongoDBRdfConfiguration,
                "Expected StatefulMongoRdfConfiguration.  Found: " + conf.getClass().getSimpleName());
        final StatefulMongoDBRdfConfiguration mongoConf = (StatefulMongoDBRdfConfiguration) conf;
        return new MongoPcjDocuments(mongoConf.getMongoClient(), mongoConf.getRyaInstanceName());
    }

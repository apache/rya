...
public class FederatedRyaExample_1 {
	...
	//	
	        ...
	        federation.addMember(createMember(numBook));
			}
    //
    // Insert 6th member seperately, all those members are having different table prefix
    //			
			test_repo= createMember(6);
			federation.addMember(test_repo);
			con = repo.getConnection();
			RepositoryConnection test_con=test_repo.getConnection();
	...
	//  Execute queries for con & test_con
			...
			TupleQuery tupleQuery_2 = test_conn.prepareTupleQuery(QueryLanguage.SPARQL,
					query);
	...
	private static Repository createMember(int ID)
	...
	private static Configuration getConf(int numTable) {

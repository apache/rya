...
public class DynamoDBRyaDirectExample {
    private static final Logger log = Logger.getLogger(DynamoDBRyaDirectExample.class);
...
//            testAddNamespaces(conn);
    ...
    private static Configuration getConf() {
        ...
        conf.set(ConfigUtils.USE_DYNAMO, "true");
        conf.set(DynamoRdfConfiguration.AWS_USER, "Fake");
        conf.set(DynamoRdfConfiguration.AWS_SECRET_KEY, "Fake");
        conf.set(DynamoRdfConfiguration.AWS_ENDPOINT, "http://localhost:8000");
        conf.set(DynamoRdfConfiguration.DYNAMO_TABLE_NAME, "rya_test1");
//        conf.set(RdfCloudTripleStoreConfiguration.CONF_INFER, "false");
        return conf;
    }
    ...
    public static void testAddAndDelete(SailRepositoryConnection conn) throws MalformedQueryException, RepositoryException,
        ...
        update.execute();
         ...
         query = "select ?s ?o { GRAPH <http://updated/test> {?s <http://acme.com/actions/likes> ?o . }}";
         ...
         query = "select ?s ?p { GRAPH <http://updated/test> {?s ?p  \"Avocados\". }}";
         resultHandler = new CountingResultHandler();
         tupleQuery = conn.prepareTupleQuery(QueryLanguage.SPARQL, query);
         tupleQuery.evaluate(resultHandler);
         log.info("Result count : " + resultHandler.getCount());

...
public class Util {
    /**
     * @return a new set which is the union of the two argument sets.
     */
    public static <T> Set<T> union(final Set<T> x, final Set<T> y) {
        final Set<T> result = CollectionFactory.createHashedSet(x);
        result.addAll(y);
        return result;
    }

    /**
     * @return a new set which contains exactly the names of the variable[ node]s
     * in the triple.
     */
    public static Set<String> variablesOf(final Triple t) {
        final Set<String> result = CollectionFactory.createHashedSet();
    ...
    private static void addIfVariable(final Set<String> result, final Node n) {
        if (n.isVariable()) {

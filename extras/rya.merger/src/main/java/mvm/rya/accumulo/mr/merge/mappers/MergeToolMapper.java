...
public class MergeToolMapper extends Mapper<Key, Value, Text, Mutation> {
    ...
    public void run(Context context) throws IOException, InterruptedException {
        ...
        try {
            RyaStatement parentRyaStatement = nextParentRyaStatement(context);
            ...
            while (compareKeysResult != CompareKeysResult.FINISHED) {
                ...
                switch (compareKeysResult) {
                    case ADVANCE_CHILD:
                        ...
                        break;
                    case ADVANCE_PARENT:
                        parentRyaStatement = nextParentRyaStatement(context);
                        break;
    ...
    private RyaStatement nextParentRyaStatement(Context context) throws IOException, InterruptedException {
        RyaStatement parentRyaStatement = null;
        if (context.nextKeyValue()) {
            Key parentKey = context.getCurrentKey();
            Value parentValue = context.getCurrentValue();
            try {
                parentRyaStatement = createRyaStatement(parentKey, parentValue, parentRyaContext);
            } catch (TripleRowResolverException e) {
                log.error("TripleRowResolverException encountered while creating parent statement", e);
            }
        }
        return parentRyaStatement;
    }

    private RyaStatement nextChildRyaStatement() {
        RyaStatement childRyaStatement = null;
        if (childIterator.hasNext()) {
            Entry<Key, Value> childEntry = childIterator.next();
            Key childKey = childEntry.getKey();
            Value childValue = childEntry.getValue();
            try {
                childRyaStatement = createRyaStatement(childKey, childValue, childRyaContext);
            } catch (TripleRowResolverException e) {
                log.error("TripleRowResolverException encountered while creating child statement", e);
            }
        }
        return childRyaStatement;
    }
    ...
    private Scanner setupChildScanner(Context context) throws IOException {
        ...
        Scanner childScanner = AccumuloRyaUtils.getScanner(childTableName, childConfig);
        childScanner.setRange(splitRange);
